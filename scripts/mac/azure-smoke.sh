...
script="\$ErrorActionPreference='Stop'; \$root=Join-Path \$env:TEMP ('rmm-issue3-'+[Guid]::NewGuid().ToString('N')); New-Item -ItemType Directory \$root | Out-Null; [IO.File]::WriteAllBytes((Join-Path \$root 'source.zip'),[Convert]::FromBase64String('$payload')); Expand-Archive (Join-Path \$root 'source.zip') (Join-Path \$root 'source'); & (Join-Path \$root 'source/scripts/windows/windows-smoke.ps1')"
