...
def test_pending_key_requires_admin_approval_then_fresh_possession_proof():
    ...
    assert httpx.post(BASE + "/pairings/approve", json={
        "code": pending["code"], "device_name": "Reception PC",
    }).status_code == 401
    approved = httpx.post(BASE + "/pairings/approve", headers=admin,
                          json={"code": pending["code"], "device_name": "Reception PC"})
    assert approved.status_code == 200
    device = approved.json()["device_id"]
    assert httpx.post(BASE + "/pairings/approve", headers=admin,
                      json={"code": pending["code"], "device_name": "Reception PC"}).status_code == 409
...
def test_two_admins_cannot_bind_one_code_to_two_workspaces():
    ...
    with ThreadPoolExecutor(2) as pool:
        responses = list(pool.map(lambda admin: httpx.post(
            BASE + "/pairings/approve", headers=admin,
            json={"code": code, "device_name": "Race PC"}), admins))
...
def test_reconnecting_during_approval_never_issues_a_replacement_code():
    for _ in range(5):
        ...
        with ThreadPoolExecutor(2) as pool:
            ...
            approval = pool.submit(httpx.post, BASE + "/pairings/approve", headers=admin,
                                   json={"code": code, "device_name": "Reconnect PC"})
...
def test_approval_guesses_are_bounded_even_with_a_valid_admin():
    ...
    for _ in range(10):
        assert httpx.post(BASE + "/pairings/approve", headers=admin,
                          json={"code": "AAAAAAAAAAAA", "device_name": "Guess PC"}).status_code == 409
    denied = httpx.post(BASE + "/pairings/approve", headers=admin,
                        json={"code": "AAAAAAAAAAAA", "device_name": "Guess PC"})
...
def test_validation_does_not_echo_secrets_or_accept_key_rebinding():
    ...
    assert httpx.post(BASE + "/pairings/approve", headers=admin,
                      json={"code": code, "device_name": "Validation PC"}).status_code == 200
...
def test_device_pagination_stays_inside_the_admins_workspace():
    ...
    for index in range(2):
        ...
        with connect(BASE.replace("http", "ws") + "/agent") as socket:
            code = prove(socket, key, public)["code"]
        assert httpx.post(BASE + "/pairings/approve", headers=admin,
                          json={"code": code, "device_name": f"Paged PC {index}"}).status_code == 200
