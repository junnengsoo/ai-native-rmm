...
waiting to 15 seconds, and idle frame waiting to two minutes. Outbound output
frames are chunked before transport. Timeout/worker-loss results conservatively
disclose possible capture loss. Success output is string rendered from PowerShell
objects; debug/verbose/progress streams are drained but not retained. Later
service lifecycle slices remain separate. Job cleanup covers inherited worker
